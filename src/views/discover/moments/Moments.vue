...
<template>
  <div class="_full-container"
    ...
    <div class="_full-content"
      ...
      <!-- refreBall -->
      <div class="moment__refresh"
           :style="refreshStyle"
           :class="{ kkk: topStatus === 'loading' }"></div>
      <!-- 单条说说 -->
      <div class="moment__wrapper"
           id="drag"
        ...
        <div id="drag-inner"
          ...
          <MomentProfile class="moment__profile"
                         @cover-click="coverDidClick"></MomentProfile>
          <div class="mh-moment"
            ...
            <div class="mh-moment__hd">
              <!-- 头像 -->
              <img :src="moment.user.profile_image_url"
                   alt
            ...
            <div class="mh-moment__bd">
              ...
              <!-- 地理位置 -->
              <div class="moment__location-wrapper"
              ...
              <div class="mh-moment__more-wrapper">
                ...
                <transition name="fade">
                  ...
                  <MomentOperationMore class="more-wrapper__operation"
                                       v-if="moment.showCmt"
                                       :thumbed="moment.attitudes_status"
                                       @thumb-click="thumbAction(moment, $event)"
                                       @comment-click="commentAction(moment)"></MomentOperationMore>
                </transition>
                <div class="mh-moment__more"
              ...
              <div class="moment__comment-wrapper"
                ...
                <div class="comment-wrapper__attitudes"
                     ...
                     @click="xxoo($event)"
                     v-if="moment.attitudes_list.length > 0"></div>
                <!-- 评论列表 -->
                <div class="comment-wrapper__comments"
                     v-if="moment.comments_list.length > 0">
                  <!-- 这里事件把 index idx 都传出去 -->
                  <div class="comment-wrapper__comment"
                       v-for="(cmt, idx) in moment.comments_list"
                       :key="idx"
                       v-html="cmt.commentHtml"
                       @click="commentItemDidClick(index, idx, $event)"></div>
                </div>
                <!-- 分割线 -->
  ...
  methods: {
    ...
    handleDomDatas(start) {
      ...
      for (let i = start; i < length; i++) {
        ...
        // 获取文本内容高度
        let contentH = window.getComputedStyle(element).height.replace('px', '')
        // 判断
        if (contentH > 5 * 20) {
          moment.unfold = false
...
.moment__refresh {
  ...
  width: 30px;
  height: 30px;
  background: url(../../../assets/images/moments/wx_album_refresh.png) no-repeat
    0 0;
  background-size: contain;
}
...
.moment__video-wrapper {
  width: 103px;
  height: 181px;
  background: url(../../../assets/images/moments/wx_video_cover.png) no-repeat 0
    0;
  background-size: contain;
}
.video-wrapper__play {
